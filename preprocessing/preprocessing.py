...
def load_and_merge_data(customer_orders_path, census_data_path):
    customer_orders = pd.read_csv(customer_orders_path, dtype={'billing_postcode': str, 'shipping_postcode': str})
    census_data = pd.read_csv(census_data_path, dtype={'zip': str})

    customer_orders['zip'] = customer_orders['billing_postcode'].str[:5]
...
def fetch_gender_data():
    ...
    X = X.sort_values(by=['Name', 'Probability'], ascending=[True, False]).drop_duplicates(subset='Name')

    gender_lookup = X.set_index('Name')[['Gender', 'Probability']].to_dict(orient='index')
...
def categorize_billing_company(df):
    df['has_billing_company'] = df['billing_company'].notna() & (df['billing_company'].str.strip() != '')

    def get_company_type(name):
        if pd.isna(name) or name.strip().lower() in ['unknown', '', 'optional', 'title (optional)']:
            return 'UNKNOWN'
...
def categorize_shipping_company(df):
    df['has_shipping_company'] = df['shipping_company'].notna() & (df['shipping_company'].str.strip() != '')
...
def drop_cols(df):
    ...
    df.drop(columns=drop_cols_extra, inplace=True)

    return df

def transform_item_features(df):
    sku_columns = [f'item_{i}_sku' for i in range(1, 8)]
    subtotal_columns = [f'item_{i}_subtotal' for i in range(1, 8)]
    quantity_columns = [f'item_{i}_quantity' for i in range(1, 8)]
    ...
    final_df = pd.concat(
        [df.drop(columns=sku_columns + subtotal_columns + quantity_columns), count_data, subtotal_data], axis=1)
...
def fix_features(df):
    ...
    drop_cols =["coupon_1_id", "coupon_2_id", "coupon_2_code","coupon_2_amount", "coupon_2_description","coupon_3_id","coupon_3_code","coupon_3_amount","coupon_3_description",	"coupon_4_id","coupon_4_code","coupon_4_amount","coupon_4_description",	"coupon_5_id","coupon_5_code", "coupon_5_amount","coupon_5_description"]
...
def main():
    data_to_predict_path = '/Users/janitaaamir/Downloads/all_data_transformed.csv'
    census_data_path = '/Users/janitaaamir/PycharmProjects/Capstone/ACSST5Y2022/filtered_data.csv'
    output_path = '/Users/janitaaamir/Downloads/all_data_transformed_encoded.csv'
    original_training_data_path = "/Users/janitaaamir/Downloads/non-encoded/encoded/combined_data_condensed.csv"
    ...
    merged_data = load_and_merge_data(data_to_predict_path, census_data_path)
    gender_lookup = fetch_gender_data()
    enriched_data = add_gender_info(merged_data, gender_lookup)

    final_data = preprocess_data(enriched_data, original_df)
    final_data.to_csv(output_path, index=False)

    print(f"Data processing complete. File saved at: {output_path}")
